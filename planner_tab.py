...
Handles grid generation, user interaction (cell clicks), solver execution, and exporting.
"""
...
class PlannerTab(ctk.CTkFrame):
    ...
    Attributes:
        ...
        last_loaded (Tuple[int, int]): Tuple of (month, year) for the currently displayed grid.
    ...
    def __init__(self, parent, config: AppConfig, on_update_callback=None):
        ...
        Args:
            ...
            on_update_callback: Optional function to call when data changes (syncs Settings).
        """
    ...
    def _build_ui(self):
        ...
        self.cmb_month = ctk.CTkComboBox(top, values=list(calendar.month_name)[1:], width=110)
        ...
        self.btn_24 = GH.create_button(top, "Check All 24H", self.toggle_all_24h, "left", 120, "#7B1FA2")
        
        # Right Side Actions (Reset, Clear, Import)
        GH.create_button(top, "Reset Table", self.reset_all, "right", 100, "#D32F2F")
        GH.create_button(top, "Clear Duties", self.clear_duties, "right", 110, "#EF5350")
        GH.create_button(top, "Import Previous Month", self.import_balances, "right", 160)
        ...
        self.btn_exp = GH.create_button(bot, "Export xlsx", self.save_excel, "right", 120, "#2E7D32")
        self.btn_exp.configure(state="disabled") # Disabled until grid loaded
        
        self.btn_run = GH.create_button(bot, "GENERATE FILL", self.run_solver, "right", 140, "#1565C0")
    ...
    def refresh_grid(self):
        ...
        try:
            ...
            if self.last_loaded == (m, y): return
            ...
            self.cells.clear(); self.day_mode_vars.clear(); self.day_active_vars.clear(); self.stat_labels.clear()
            
            # 1. Build Header Rows
            headers = ["Date", "Day", "Duty?", "24H?", "NAME"]
            for i, t in enumerate(headers):
                GH.create_grid_header(self.scroll, t, i, 0, width=(80 if i==4 else 50))
            ...
            for d in range(1, days+1):
                ...
                ctk.CTkCheckBox(
                    self.scroll, text="", variable=avar, width=20, 
                    command=lambda x=d: self.toggle_active(x)
                ).grid(row=2, column=d)
                
                # Row 3: 24H Toggle
                # Logic: Default to 24H only if it's a Public Holiday. Weekends default to Shift.
                default_mode = C.ScheduleMode.FULL_24H.value if is_ph else C.ScheduleMode.SHIFT.value
                mvar = ctk.StringVar(value=default_mode)
                self.day_mode_vars[d] = mvar
                ctk.CTkCheckBox(
                    self.scroll, text="", variable=mvar, 
                    onvalue=C.ScheduleMode.FULL_24H.value, 
                    offvalue=C.ScheduleMode.SHIFT.value, width=20
                ).grid(row=3, column=d)
            ...
            for idx, p in enumerate(sorted(self.config.personnel)):
                r = idx + 5
                ctk.CTkLabel(self.scroll, text=p).grid(row=r, column=0, sticky="w", padx=5)
    ...
    def toggle_all_24h(self):
        ...
        v = C.ScheduleMode.FULL_24H.value if self.all_24h_active else C.ScheduleMode.SHIFT.value
        for var in self.day_mode_vars.values(): var.set(v)
        ...
        try:
            ...
            for p in self.config.personnel:
                ...
                for d in range(1, self.current_days+1):
                    # Skip disabled days
                    if not self.day_active_vars[d].get(): continue
    ...
    def _success(self, res):
        ...
        if res:
            ...
            for (p, d), v in sched.items():
                if (p, d) in self.cells: self.cells[(p, d)].set_val(v)
            self.recalculate()
            self.lbl_stat.configure(text="Done")
        else: 
            self.lbl_stat.configure(text="Failed")
            messagebox.showwarning("Solver", "No solution found. Check your constraints.")
    ...
    def reset_all(self):
        ...
        if messagebox.askyesno("Confirm", "Reset entire table? All data will be lost."):
            for c in self.cells.values(): c.set_val("")
            self.recalculate()

    def clear_duties(self):
        """Clears only duties (AM/PM/24H/SB) but preserves Leaves (X)."""
        if messagebox.askyesno("Confirm", "Clear assigned duties? (Leaves will remain)"):
            for c in self.cells.values():
                if c.current_val in C.ACTIVE_DUTIES: c.set_val("")
            self.recalculate()
    ...
    def import_balances(self):
        ...
        Feature:
        - Overwrites the current personnel list in settings with the names found in the file.
        ...
        if fp:
            ...
            msg = (f"Found {len(imported_names)} names in file.\n\n"
